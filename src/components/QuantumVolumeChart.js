...
class QuantumVolumeChart extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      d: [],
      ...
      areLabelsArxiv: false
    }

    this.onLabelSwitchClick = this.onLabelSwitchClick.bind(this)
    this.onArxivSwitchClick = this.onArxivSwitchClick.bind(this)
    this.refreshLabels = this.refreshLabels.bind(this)
    this.onScaleSwitchClick = this.onScaleSwitchClick.bind(this)
    this.onMetricSelectChange = this.onMetricSelectChange.bind(this)
    this.redraw = this.redraw.bind(this)
    this.plot = this.plot.bind(this)
    this.barplot = this.barplot.bind(this)
  ...
  onLabelSwitchClick () {
    let alv = !(this.state.areLabelsVisible)
    this.setState({ areLabelsVisible: alv })
    this.refreshLabels(alv)
  }

  onArxivSwitchClick () {
    this.setState({ areLabelsArxiv: !(this.state.areLabelsArxiv) })
    this.refreshLabels(this.state.areLabelsVisible)
  }
  ...
  onScaleSwitchClick () {
    this.setState({ isScaleLinear : !(this.state.isScaleLinear) })
    this.redraw(this.state.d, this.state.isScaleLinear, this.state.areLabelsArxiv, this.state.areLabelsVisible, this.state.isQubits, this.state.metricName)
  }

  onMetricSelectChange (e) {
    this.setState({ metricName: e.target.value })
    this.redraw(this.state.d, this.state.isScaleLinear, this.state.areLabelsArxiv, this.state.areLabelsVisible, this.state.isQubits, this.state.metricName)
  }

  redraw (d, isScaleLinear, areLabelsArxiv, areLabelsVisible, isQubits, metricName) {
    ...
    this.plot(d, isScaleLinear, areLabelsArxiv, areLabelsVisible, isQubits, metricName)
    this.legend(this.state.isQubits)
  ...
  plot (
    ...
    const metricNameInvariant = this.state.metricName.toLowerCase()
  ...
  legend (isQubits, circleSizeFields = 8) {
    let multCoeff = 1
    if (isMobile) {
      multCoeff = 1.5
    }
  ...
  componentDidMount () {
    ...
    const taskRoute = config.api.getUriPrefix() + '/task/' + this.state.taskId
    axios.get(taskRoute)
      .then(res => {
        const task = res.data.data
        this.setState({ taskName: task.fullName })
        task.childTasks.sort(sortByCounts)
        if (this.state.onLoadData) {
          this.state.onLoadData(task)
        }
        ...
        for (let i = 0; i < results.length; ++i) {
          ...
          if (metricNames.includes(results[i].metricName)) {
            ...
            } else if (this.state.isQubits && (results[i].metricName.toLowerCase() === 'iterations to 1e-3 error')) {
        ...
        const d = results
          ...
          .sort((a, b) => (this.state.taskId === 119) ? (a.metricValue > b.metricValue) : this.state.isQubits ? (a.qubitCount < b.qubitCount) : (a.dayIndexInEpoch > b.dayIndexInEpoch))
          this.setState({ metricNames, metricName, d })
        this.redraw(d, this.state.isScaleLinear, this.state.areLabelsArxiv, this.state.areLabelsVisible, this.state.isQubits, metricName)
      })
      .catch(err => {
        console.log(err)
      })
      ...
      <span>
        <div className='row'>
          <div className='col text-start'>
            <h4 align='left'>{this.state.taskName}</h4>
          </div>
        </div>
        <div id='cargo'>
          <div id='my_dataviz' />
          <div id='legend_guide'>
            <div>
                ...
                <span>
                  <div id='legend-switch' style={{ marginTop: '10px' }}>
                    <label className='switch'>
                      <input id='labelSwitch' type='checkbox' onClick={this.onLabelSwitchClick} />
                  ...
                  <div id='legend-switch' style={{ marginTop: '10px' }}>
                    <label className='switch'>
                      <input id='arXivSwitch' type='checkbox' onClick={this.onArxivSwitchClick} />
                      <span className='slider round' />
                    </label>
              ...
              <div id='legend-switch' style={{ marginTop: '10px' }}>
                <label className='switch'>
                  <input id='isScaleLinearSwitch' type='checkbox' onClick={this.onScaleSwitchClick} />
                  <span className='slider round' />
                </label>
                <span className='legendTitle'>{this.state.isScaleLinear ? 'Linear | Log' : 'Log | Linear'}</span>
              </div>
              <div id='legend-switch' style={{ marginTop: '10px' }}>
                <label className='switch' style={{ width: '50%' }}>
                  <select id='metricSelect' style={{ width: '100%' }} onChange={this.onMetricSelectChange}>
                    {this.state.metricNames.map((option, index) => <option key={index} value={option}>{option}</option>)}
                  </select>
                </label>
            ...
            <div>
              <span className='legendTitle'>{this.state.isQubits ? 'Qubits' : 'Providers'}</span>
